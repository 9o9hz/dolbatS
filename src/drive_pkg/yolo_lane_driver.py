#!/usr/bin/env python3
"""YOLO segmentation lane detection and local-path generation for ROS 2.
...
class SegmentationLaneProcessor:
    ...
    def __init__(self, model_path: Path, config: LaneConfig) -> None:
        self.config = config
        ...
        if self.model.task != "segment":
            raise ValueError(
                f"best1.pt must be a segmentation model, got {self.model.task!r}"
            )
