...
DEFAULT_BEV = PROJECT_ROOT / "utils" / "bev(0729).npz"
DEFAULT_CALIBRATION = PROJECT_ROOT / "utils" / "camera_calibration.npz"
...
def load_bev(path: Path):
    with np.load(path, allow_pickle=False) as data:
        ...
        coordinate_space = (
            str(np.asarray(data["coordinate_space"]).reshape(-1)[0])
            if "coordinate_space" in data.files
            else None
        )
        if coordinate_space not in (
            ...
            raise ValueError(
                "Unsupported BEV coordinate_space: "
                f"{coordinate_space!r}. Expected "
                "'full_undistorted_camera_image'."
            )
        ...
        if "checkerboard_size" in data.files:
            ...
            pattern = None
        square_size_mm = (
            float(data["square_size_mm"])
            if "square_size_mm" in data.files
            else None
        )

    if width <= 0 or height <= 0:
        raise ValueError(f"Invalid BEV output size: {width}x{height}")
    if homography is not None and (
        homography.shape != (3, 3)
        or not np.all(np.isfinite(homography))
    ):
        raise ValueError("BEV homography must be a finite 3x3 matrix")
...
def load_calibration(path: Optional[Path]):
    if path is None:
        return None
    with np.load(path, allow_pickle=False) as data:
        camera = np.asarray(data["camera_matrix"], np.float64)
        distortion = np.asarray(
            data["distortion_coefficients"], np.float64
        )
        new_camera = np.asarray(
            data["new_camera_matrix"]
            if "new_camera_matrix" in data.files
            else camera,
            np.float64,
        )
        pattern = (
            _scalar(data, ("checkerboard_columns",), DEFAULT_PATTERN[0]),
            _scalar(data, ("checkerboard_rows",), DEFAULT_PATTERN[1]),
        )
        square_size_mm = (
            float(data["square_size_mm"])
            if "square_size_mm" in data.files
        ...
        width = _scalar(data, ("image_width",), 640)
        height = _scalar(data, ("image_height",), 480)
    if (
        camera.shape != (3, 3)
        or new_camera.shape != (3, 3)
...
def analyze_checkerboard(
    ...
    cell_width = (
        horizontal_spacing[:-1, :] + horizontal_spacing[1:, :]
    ) * 0.5
    cell_height = (
        vertical_spacing[:, :-1] + vertical_spacing[:, 1:]
    ) * 0.5
    cell_ratios = cell_width / np.maximum(cell_height, 1e-9)
    cell_ratio_errors = np.abs(cell_ratios - 1.0) * 100.0
    cell_sizes = np.sqrt(np.maximum(cell_width * cell_height, 0.0))
    ...
    return {
        ...
        "cell_ratios_x_over_y": cell_ratios.tolist(),
        "mean_cell_square_error_percent": float(
            np.mean(cell_ratio_errors)
        ),
        "max_cell_square_error_percent": float(
            np.max(cell_ratio_errors)
        ),
        "cell_size_cv_percent": coefficient_of_variation(cell_sizes),
...
def draw_report(
    ...
    lines = [
        f"cell square error: mean "
        f"{report['mean_cell_square_error_percent']:.2f}% "
        f"max {report['max_cell_square_error_percent']:.2f}%",
        f"global X/Y {report['square_ratio_x_over_y']:.4f} "
        f"| cell size CV {report['cell_size_cv_percent']:.2f}%",
...
class BevCheckerboardDistortionNode(Node):
    def __init__(self, arguments: argparse.Namespace) -> None:
        ...
        if arguments.pattern is not None:
            pattern = tuple(arguments.pattern)
        elif self.bev["pattern"] is not None:
            pattern = self.bev["pattern"]
        elif self.calibration is not None:
            pattern = self.calibration["pattern"]
        else:
            pattern = DEFAULT_PATTERN
        self.bev["pattern"] = pattern
        if (
            self.bev["square_size_mm"] is None
            and self.calibration is not None
        ):
            self.bev["square_size_mm"] = self.calibration["square_size_mm"]
    ...
    def _undistort(self, frame: np.ndarray) -> np.ndarray:
        ...
        new_camera = self.calibration["new_camera"]
    ...
    def on_image(self, message) -> None:
        ...
        if found:
            ...
            self.last_report.update(
                {
                    ...
                    "input_topic": self.arguments.topic,
                    "calibration_file": (
                        str(self.arguments.calibration)
                        if self.arguments.calibration is not None
                        else None
                    ),
                    "coordinate_space": self.bev["coordinate_space"],
                }
            )
            ...
            if now - self.last_log_time >= 1.0:
                self.get_logger().info(
                    "cell square error mean/max="
                    f"{self.last_report['mean_cell_square_error_percent']:.2f}/"
                    f"{self.last_report['max_cell_square_error_percent']:.2f}%, "
                    "spacing CV X/Y="
...
def parse_arguments(argv=None):
    ...
    parser.add_argument(
        ...
        help=f"ROS image topic (default: {DEFAULT_TOPIC})",
    )
    parser.add_argument(
