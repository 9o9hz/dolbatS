...
DEFAULT_BEV = PROJECT_ROOT / "bev(0729).npz"
...
def absolute_project_path(path: Path) -> Path:
    ...
    if not result.is_absolute():
        # Accept paths relative to the caller's current directory first
        # (for example ``utils/bev(0729).npz`` from the repository root).
        cwd_path = Path.cwd() / result
        result = cwd_path if cwd_path.exists() else PROJECT_ROOT / result
