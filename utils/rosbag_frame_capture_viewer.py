...
class RosbagImageSource:
    ...
    def _detect_storage_id(bag_path: Path) -> str:
        """Determine the rosbag storage plugin without relying on auto-detection."""
        ...
        if metadata_path.is_file():
            for line in metadata_path.read_text(
                ...
                if separator and key.strip() == "storage_identifier":
                    storage_id = value.strip().strip("\"'")
                    if storage_id:
                        return storage_id
        ...
        if bag_path.is_file():
            ...
            candidates = list(bag_path.iterdir())
        if any(path.suffix == ".db3" for path in candidates):
            return "sqlite3"
        if any(path.suffix == ".mcap" for path in candidates):
            return "mcap"
        raise ValueError(
            "rosbag 저장 형식을 판단할 수 없습니다. "
            f"metadata.yaml, .db3 또는 .mcap 파일을 확인하세요: {bag_path}"
        )
